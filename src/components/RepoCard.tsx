...
const RepoCard = ({ repo, showOwner = false, dateFormat = {year: 'numeric', month: 'long', day: 'numeric'}, variant = 'default' }: RepoCardProps) => {
    const isEnhanced = variant === 'enhanced';
    const baseClasses = isEnhanced ? 'bg-gray-800 rounded-xl p-6 hover:bg-gray-750 transition-all duration-200 border border-gray-700 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/20' : 'bg-gray-800 rounded-lg p-6 hover:bg-gray-700 transition-colors border border-gray-700';

    const descriptionClamp = isEnhanced ? 'line-clamp-3' : 'line-clamp-2';
    const statLayout = isEnhanced ? 'flex flex-warp items-center gap-4 text-sm text-gray-500' : 'flex items-center gap-4 text-sm text-gray-500';
                ...
                {isEnhanced ? (
                        <div className='flex-1 min-w-0'>
                            <h3 className='text-lg font-semibold text-white mb-1 truncate'>
                                <a href={repo.html_url} target='_blank' rel='noopener noreferrer' className={linkClass}>{repo.full_name || repo.name}</a>
                            </h3>
