...
import { FaMapMarker, FaBuilding, FaLink, FaCalendarAlt, FaGithub, FaUsers, FaUserPlus, FaCode, FaFileCode } from "react-icons/fa";
import type { GitHubProfile, GithubRepo } from "../models/github";
...
const ProfileViewer = () => {
    ...
    const stats = [
        ...
        {icon: FaFileCode, label: 'Gists', value: profile?.public_gists, bgClass: 'from-orange-600/20 to-blue-800/20', borderClass: 'border-orange-500/30 hover:border-orange-400/50', iconClass: 'text-orange-400'}
    ];

    const infoItems = [
        {condition: profile?.company, icon: FaBuilding, label: 'Company', value: profile?.company, iconClass: 'text-blue-400', isLink: true},
        {condition: profile?.location, icon: FaMapMarker, label: 'Location', value: profile?.location, iconClass: 'text-red-400', isLink: true},
            ...
            <div className={`max-w-6xl mx-auto ${hasContent ? '' : 'w-full'}`}>
                ...
                {profile && (
                    <div className="bg-linear-to-br from-gray-800 via-gray-800 to-gray-900 rounded-2xl shadow-2xl overflow-hidden mb-8 border border-gray-700/50 backdrop-blur-sm">
                        <div className="bg-linear-to-b from-blue-600/20 via-purple-600/20 to pink-600/20 p-6 border-b border-gray-700">
                        ...
                        <div className="p-6 bg-gray-900/50">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                                {stats.map(({ icon: Icon, label, value, bgClass, borderClass, iconClass }) => (
                                    <div key={label} className={`bg-linear-to-br ${bgClass} p-4 rounded-xl border ${borderClass} transition-all duration-200 hocer:scale-105`}>
                                ...
                                {infoItems.map(({condition, icon: Icon, label, value, iconClass, isLink}) => condition && (
                                    <div key={label} className="flex items-center gap-3 text-gray-300 p-3 rounded-lg bg-gray-800/50 hover:bg-gray-800 transition-colors">
                                        ...
                                        <div className={isLink ? "flex-1 min-w-0" : ""}>
                                            ...
                                            {isLink ? <a href={value?.startsWith('https://') ? value : `https://` + value} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline font-semibold truncate block">{value}</a> : <div className="font-semibold">{value}</div>}
                                        </div>
                                    </div>
                                ))}
